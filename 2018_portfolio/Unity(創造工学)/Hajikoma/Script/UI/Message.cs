...
public class Message : MonoBehaviour
{
    ...
    void Start()
    {
        master = GameObject.Find("GameObject").GetComponent<MasterStreetView>();

        buildImage = transform.Find("Panel/Image_buliding").GetComponent<Image>();
        buildImage.enabled = false;
        clickIcon = transform.Find("Panel/Image").GetComponent<Image>();
        clickIcon.enabled = false;
        messageText = transform.Find("Panel/Text").GetComponent<Text>();
        messageText.text = "";
        ...
        else
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (nowTextNum >= message.Length)
                {
                    ...
                    Debug.Log("メッセージ終了");
                    master.receiveScriptEnabledFlag = true;
                    master.receiveMessageFlag = true;
                    //master.receiveInfoButtonFlag = false;
    ...
    //spriteVer
    Sprite texture;
    void setImage(string buildName)
    {
        this.buildName = buildName;

        //material = sphere.GetComponent<Renderer>().material;
        Debug.Log("setImage" + buildName);
        texture = Resources.Load<Sprite>("Image/DetailBulid/" + buildName);
        buildImage.sprite = texture;//写真セット
        buildImage.enabled = true;
    }
    //　他のスクリプトから新しいメッセージを設定
    private string buildName;
    public void setMessagePanel(string message, string buildName)
    {
        master.receiveScriptEnabledFlag = false;//スクリプトOFFにしてマウスで回転できないようにする　//TOUCHRAYでクリックされるとスクリプトON
        setMessage(message);
        setImage(buildName);
